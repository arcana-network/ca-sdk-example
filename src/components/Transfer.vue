...
import { AllowanceHookSources } from '@arcana/ca-sdk';
...
const expectedStepsListener = (data: any) => {
  console.log("Expected steps", data)
  state.value.steps = data.map((s: ProgressStep) => ({ ...s, done: false }))
  state.value.inProgress = true
}

const stepDoneListener = (data: any) => {
...
onUnmounted(() => {
  if (ca) {
    ca.caEvents.removeListener("expected_steps", expectedStepsListener)
    ca.caEvents.removeListener("step_complete", stepDoneListener)
  }
})
onMounted(async () => {
  ca = await getCA()
  ca.caEvents.addListener("expected_steps", expectedStepsListener)
  ca.caEvents.addListener("step_complete", stepDoneListener)
  ...
  ca.setOnIntentHook(({ intent, allow, deny, refresh }) => {
    ...
    intentModal.value.intervalHandler =
      setAsyncInterval(async () => {
        ...
        intentModal.value.intent = await refresh()
...
const submitAllowance = () => {
  ...
  if (allowanceModal.value.allow) {
    const values = allowanceModal.value.data.map(() => "max")
  ...
  try {
    ...
    if (ca) {
      const handler = await ca.transfer({
        ...
        chainID: Number(transferValue.value.chain),
...
const handleSimulateTransfer = async () => {
  transferValue.value.simulationLoading = true
  try {
    console.log({ transferValue: transferValue.value })
    if (ca) {
      const handler = await ca.transfer({
      ...
      const simulation = await handler.simulate();
    ...
    transferValue.value.simulationLoading = false
  }
}
...
const handleBridge = async () => {
  ...
  try {
    ...
    if (ca) {
      const b = await ca.bridge({
        amount: bridgeValue.value.amount,
        chainID: Number(bridgeValue.value.chain),
...
const handleSimulateBridge = async () => {
  bridgeValue.value.simulationLoading = true
  try {
    console.log({ bridgeValue: bridgeValue.value })
    if (ca) {
      const b = await ca.bridge({
      ...
      const simulation = await b.simulate();
      console.log({ simulation })
    }
  } catch (e) {
    resetState()
    ...
    // messages.value.errorMsg = e.message
    createErrorToast(e)
    console.error("Bridge simulation failed with error", e);
  } finally {
    bridgeValue.value.simulationLoading = false
  }
}
  ...
  <div v-if="!state.inProgress" class="space-x-4">
    <div v-if="props.currentTab === 'transfer'"
      ...
      <div class="self-end">
        ...
        <button @click="handleSimulateTransfer" :disabled="transferValue.simulationLoading"
          class="ml-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
          <svg v-if="transferValue.simulationLoading" aria-hidden="true" role="status"
            class="inline w-4 h-4 me-3 text-white animate-spin" viewBox="0 0 100 101" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
              d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
    ...
    <div v-if="props.currentTab === 'bridge'"
      ...
      <button @click="handleSimulateBridge" :disabled="bridgeValue.simulationLoading"
        class="ml-2 text-right text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
        <svg v-if="bridgeValue.simulationLoading" aria-hidden="true" role="status"
          class="inline w-4 h-4 me-3 text-white animate-spin" viewBox="0 0 100 101" fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            <tr v-for="elem in allowanceModal.data" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              ...
              <td class="px-6 py-4">{{ `${elem.chain.name}(${elem.chain.id})` }}</td>
              <td class="px-6 py-4">{{ elem.allowance.current.toString().startsWith("11579208923731619542") ? "MAX" :
                elem.allowance.current }}</td>
              <td class="px-6 py-4">
                {{ elem.allowance.minimum }}
              </td>
              <td class="px-6 py-4">
          ...
          <div class="p-2 text-gray-900 dark:text-white " v-if="intentModal.feesBreakupOpen">
            <div class="p-5 w-full bg-gray-200 flex flex-col space-y-6 dark:bg-gray-900 font-xs rounded">
              ...
              <div class="flex">
                ...
                <div class="basis-1/2 text-xs text-right">
                  {{ `${intentModal.intent?.fees.gasSupplied} ${intentModal.intent?.token.symbol}` }}
                </div>
              </div>
            </div>
          </div>
