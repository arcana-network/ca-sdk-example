...
const eventListener = (data: any) => {
  switch (data.type) {
    case "EXPECTED_STEPS": {
      ...
      state.value.steps = data.data.map((s: ProgressStep) => ({
        ...s,
        done: false,
      }));
...
onMounted(async () => {
  ...
  ca.setOnIntentHook(({ intent, allow, deny, refresh }) => {
    ...
    setTimeout(() => {
      intentModal.value.intervalHandler = setAsyncInterval(async () => {
...
<template>
  ...
  <div v-if="!state.inProgress" class="space-x-4">
    <div
      v-if="props.currentTab === 'transfer'"
    ...
    >
      ...
      <div class="mb-5">
        ...
        <select
          id="chains"
          name="Chain"
          v-model="transferValue.chain"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        >
          <option disabled value="0">Select chain</option>
          <option v-for="chain in CHAINS" :value="chain.chainID">
            {{ chain.chainName }}
          </option>
        </select>
      </div>
  ...
  <div v-if="state.inProgress" class="mx-auto">
    ...
    <div class="w-full flex space-x-4 mb-6">
      ...
      >
        <li
          v-for="s in state.steps"
          class="flex items-center py-6 px-5 font-medium text-gray-900 dark:text-white"
        >
          <svg
            v-if="s.done"
          ...
          <svg
            v-else
            aria-hidden="true"
            class="w-5 h-5 me-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
            viewBox="0 0 100 101"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
          ...
          {{ getTextFromStep(s as any, false) }}
        </li>
      </ul>
    </div>
    <button
  ...
  <Modal
    :isOpen="allowanceModal.open"
    @modal-close="rejectAllowance"
    @submit="submitAllowance"
    name="allowance-modal"
  >
    <template #header>
    ...
    <template #content>
      <div class="text-xl font-semibold text-gray-900 dark:text-white">
        ...
        >
          ...
          <tbody>
            <tr
              v-for="elem in allowanceModal.data"
              class="bg-white border-b dark:bg-gray-800 dark:border-gray-700"
            >
              <td class="px-6 py-4">{{ elem.token.symbol }}</td>
              <td class="px-6 py-4">
                {{ `${elem.chainName}(${elem.chainID})` }}
              </td>
              <td class="px-6 py-4">
                {{
                  elem.currentAllowance
                    .toString()
                    .startsWith("11579208923731619542")
                    ? "MAX"
                    : elem.currentAllowance
                }}
              </td>
  ...
  <Modal
    :isDisabled="intentModal.intentRefreshing"
    :isOpen="intentModal.open"
    @modal-close="rejectIntent"
    @submit="allowIntent"
    name="intent-modal"
  >
    <template #header>
      ...
      <div class="p-3">
        <div>
          <div class="flex text-lg text-gray-900 dark:text-white p-2">
            ...
            <div class="basis-1/2 text-right text-sm">
              {{
                `${intentModal.intent?.destination.chainName}
              (${intentModal.intent?.destination.chainID})`
              }}
            </div>
          </div>
          <div class="flex text-lg text-gray-900 dark:text-white p-2">
            ...
            <div class="basis-1/2 text-right flex items-end flex-col">
              <p class="text-sm">
                {{
                  `${intentModal.intent?.sourcesTotal} ${intentModal.intent?.token.symbol}`
                }}
              </p>
              <div class="text-right" data-accordion="collapse">
                <button
                  type="button"
                  class="flex align-right items-center justify-between p-1"
          ...
          <div
            ...
            v-if="intentModal.sourcesOpen"
          >
            <div
              class="p-5 bg-gray-200 w-full flex flex-col space-y-6 dark:bg-gray-900 font-xs rounded"
            >
          ...
          <div class="flex text-lg text-gray-900 dark:text-white p-2">
            ...
            <div class="basis-1/2 text-right text-sm flex items-end flex-col">
              <p>
                {{
                  `${intentModal.intent?.fees.total} ${intentModal.intent?.token.symbol}`
                }}
              </p>
              <div class="text-right" data-accordion="collapse">
                <button
                  ...
                  class="flex align-right items-center justify-between p-1"
                  @click="
                    intentModal.feesBreakupOpen = !intentModal.feesBreakupOpen
                  "
                >
                  <span class="text-right text-xs text-gray-400"
                    >View breakdown</span
          ...
          <div
            class="p-2 text-gray-900 dark:text-white"
            v-if="intentModal.feesBreakupOpen"
          >
            <div
              class="p-5 w-full bg-gray-200 flex flex-col space-y-6 dark: bg-gray-900 font-xs rounded"
            >
              <div class="flex">
                <div class="basis-1/2 text-xs">CA Gas Fees:</div>
                <div class="basis-1/2 text-xs text-right">
                  {{
              ...
              <div class="flex">
                <div class="basis-1/2 text-xs">Solver Fees:</div>
                <div class="basis-1/2 text-xs text-right">
                  {{
              ...
              <div class="flex">
                <div class="basis-1/2 text-xs">Protocol Fees:</div>
                <div class="basis-1/2 text-xs text-right">
                  {{
              ...
              <div class="flex">
                <div class="basis-1/2 text-xs">Gas supplied:</div>
                <div class="basis-1/2 text-xs text-right">
